...
public class ResourceControllerIT {
	...
	void doHome() throws Exception {
		mockMvc.perform(get(RESOURCE_MAPPING+"hello")).andDo(print()).andExpect(status().isOk()).andExpect(content().string("Hello Docker World"));		
	}
	...
	void doGetCodecTypes() throws Exception {
		String name = "file_example_MP4_480_1_5MG.mp4";
		mockMvc.perform(get(RESOURCE_MAPPING+"{name}/codec-types", name)).andDo(print())
			.andExpect(status().isOk()).andExpect(jsonPath("$[0].name").value("h264"))	
			.andExpect(status().isOk()).andExpect(jsonPath("$[1].name").value("aac"));
